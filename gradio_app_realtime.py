...
# 隐藏页脚;WebRTC 组件内部视频元素给温和的高度上限(不动 position/width,
# 强改会破坏组件内部布局——实测)。控件区与视频区改为上下结构,视频再大
# 也盖不住按钮。
_CSS = """
footer {display: none !important}
#rt-webrtc video {max-height: 62vh !important; object-fit: contain;}
"""

with gr.Blocks(title="SoulX-FlashHead 实时数字人", theme=gr.themes.Soft(), css=_CSS) as app:
    gr.Markdown("# 🎙️ SoulX-FlashHead 实时数字人（麦克风直驱）")
    gr.Markdown("加载模型 → 点击 Record 授权麦克风与摄像头（摄像头画面会被丢弃，属组件限制）→ "
                "开口说话。首次说话后有约 1 分钟编译预热（仅一次），之后实时对口型。")

    with gr.Row():
        with gr.Column(scale=1):
            cond_image_input = gr.Image(label="Condition Image", type="filepath",
                                        value="examples/girl.png", height=220)
        with gr.Column(scale=1):
